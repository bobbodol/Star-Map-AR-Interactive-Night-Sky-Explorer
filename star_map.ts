# star_map.ts
/**
 * 🌟 Star Map AR – Interactive Night Sky Explorer (TypeScript Edition)
 * Fully typed, advanced: ASCII sky map, constellation tracing, star search, favourites, simulation
 */
...
type Star = [string, number, number, number, string]; // name, RA(h), Dec(°), Mag, Constellation
...
class UserData {
    ...
    public latitude: number = 40.7;
    public longitude: number = -74.0;
    public timezone: number = -5;

    constructor() {
        const dir = path.join(os.homedir(), '.star_map');
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        this.filePath = path.join(dir, 'user.json');
        this._load();
    }
    ...
    private _load() {
        if (fs.existsSync(this.filePath)) {
            try {
                const data: UserDataJson = JSON.parse(fs.readFileSync(this.filePath, 'utf8'));
    ...
    isFavourite(name: string): boolean { return this.favourites.includes(name); }
}

// ─── Sky Map Engine ─────────────────────────────────────────────────────────

class SkyMap {
    private latRad: number;
    private lonRad: number;
    private timezone: number;
    private now: Date;
    public stars: Star[];

    constructor(lat: number = 40.7, lon: number = -74.0, tz: number = -5) {
    ...
    setTime(dt: Date) { this.now = dt; }

    private _julianDate(dt: Date): number {
    ...
    private _localSiderealTime(dt: Date): number {
        const jd = this._julianDate(dt);
        let gmst = 280.46061837 + 360.98564736629 * (jd - 2451545.0) + 0.000387933 * (jd - 2451545.0) ** 2;
        gmst = gmst % 360;
        let lst = gmst + this.lonRad * 180 / Math.PI;
        lst = lst % 360;
        return lst / 15.0;
    }

    private _altaz(raHours: number, decDeg: number, dt: Date): { alt: number; az: number } {
    ...
    getVisibleStars(minAlt: number = -10): { name: string; ra: number; dec: number; mag: number; constel: string; alt: number; az: number }[] {
        const visible = [];
    ...
    renderMap(width: number = 50, height: number = 20, highlightConst: string | null = null): string {
...
class StarApp {
    ...
    private sky: SkyMap;
    private highlightConst: string | null = null;
    ...
    private async highlightConstellation() {
